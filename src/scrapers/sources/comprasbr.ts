...
export async function comprasBrScraper() {
  const browser = await chromium.launch({ headless: true }); const context = await browser.newContext({ acceptDownloads: true, viewport: { width: 1440, height: 1000 } }); const page = await context.newPage(); const seen = new Set<string>(); const items: EditalRichItem[] = []; let pagesScanned = 0; let itemsSeen = 0;
  try {
    for (let pagina = 1; pagina <= MAX_PAGES; pagina++) {
      const url = COMPRAS_BR_LIST_URL; let success = false;
      for (let attempt = 0; attempt < 3 && !success; attempt++) { try { await page.goto(url, { waitUntil: "domcontentloaded", timeout: 60000 }); let frame = await waitFrame(page); for (let currentPage = 1; currentPage <= pagina; currentPage++) { const list = await extractComprasBrList(frame, currentPage); if (!list.length) { pagesScanned = Math.max(pagesScanned, currentPage - 1); if (currentPage === 1) console.log(JSON.stringify({ source: "COMPRAS_BR", success: true, items: [], reason: "no_results" })); return { pagesScanned, itemsSeen, itemsInserted: 0, documentsDownloaded: 0 }; } if (currentPage === pagina) { pagesScanned = Math.max(pagesScanned, currentPage); itemsSeen += list.length; for (const listItem of list) { const button = frame.locator(".processo-card-acessar").nth(listItem.indice); await button.click(); await page.waitForTimeout(500); const detailFrame = await waitFrame(page); if (!detailFrame.url().includes("detalhe")) throw new Error("processo não navegou para detalhe"); const detail = await extractComprasBrDetail(page, detailFrame, listItem); if (!seen.has(detail.canonicalKey || detail.link)) { seen.add(detail.canonicalKey || detail.link); items.push(detail); } await page.goto(url, { waitUntil: "domcontentloaded", timeout: 60000 }); frame = await waitFrame(page); } } if (currentPage < pagina) { const next = frame.getByText(String(currentPage + 1), { exact: true }).last(); if (!(await next.count())) return { pagesScanned, itemsSeen, itemsInserted: 0, documentsDownloaded: 0 }; await next.click(); frame = await waitFrame(page); await frame.locator(".processo-card-acessar").first().waitFor({ state: "visible", timeout: 30000 }); } } success = true; } catch (error) { if (attempt === 2) console.error(`[compras_br] página ${pagina} falhou: ${String(error)}`); else await new Promise((resolve) => setTimeout(resolve, 1000 * (attempt + 1))); } }
      if (!success) continue;
    }
    if (items.length) { const result = await upsertEditaisFromList("COMPRAS_BR", items); const documentsDownloaded = items.reduce((total, item) => total + (item.arquivos || []).filter((file) => file.status_download === "complete").length, 0); console.log(`[compras_br] páginas=${pagesScanned} vistos=${itemsSeen} itens=${items.length} upsert=${result.inserted}`); return { pagesScanned, itemsSeen, itemsInserted: result.inserted, documentsDownloaded }; }
    return { pagesScanned, itemsSeen, itemsInserted: 0, documentsDownloaded: 0 };
